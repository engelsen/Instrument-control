        ...
        function fitTrace(this)
            switch this.fit_name
                ...
                case {'Lorentzian','LorentzianGrad','Gaussian'}
                    offset=median(this.Data.x);
                    this.Data.x=this.Data.x-offset;
                    this.init_params(3)=this.init_params(3)-offset;
                    doFit(this);
                    this.coeffs(3)=this.coeffs(3)+offset;
                case {'DoubleLorentzian','DoubleLorentzianGrad'}
%                     offset=median(this.Data.x);
%                     this.Data.x=this.Data.x-offset;
%                     this.init_params(3)=this.init_params(3)-offset;
%                     this.init_params(6)=this.init_params(6)-offset;
                    doFit(this);
%                     this.coeffs(3)=this.coeffs(3)+offset;
%                     this.coeffs(6)=this.coeffs(6)+offset;
                case {'Exponential','Gorodetsky2000'}
                    doFit(this);
                otherwise
    ...
    methods(Access=private)
        ...
        function createFitStruct(this)
            ...
            addFit(this,'Gorodetsky2000',...
                'a*abs( (k0^2 - kex^2 + gamma^2 - (x-b).^2 + 2i*k0.*(x-b))./( (k0 + kex)^2 + gamma^2 - (x-b).^2 + 2i.*(x-b)*(k0 + kex) )).^2',...
                '$$a\left|\frac{\kappa_0^2-\kappa_{ex}^2+\gamma^2-(x-b)^2+2i\kappa_0(x-b)}{(\kappa_0+\kappa_{ex})^2+\gamma^2-(x-b)^2+2i(x-b)(\kappa_0+\kappa_{ex})}\right|^2$$',...
            { 'a','b', 'gamma','k0', 'kex'},...
                {'Background','Center','Mode splitting','Intrinsic','Extrinsic'});
