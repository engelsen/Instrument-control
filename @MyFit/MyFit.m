    ...
    methods(Access=private)
        ...
        function createFitStruct(this)
            ...
            addFit(this,'Lorentzian','1/pi*a*b/2/((x-c)^2+(b/2)^2)+d',...
                ...
                {'Amplitude','Width','Center','Offset'});
            addFit(this,'LorentzianGrad','1/pi*a*b/2/((x-c)^2+(b/2)^2)+d*x+e',...
            ...
            addFit(this,'Gorodetsky2000',...
                'a*abs( (k0^2 - kex^2 + gamma^2 - (x-d).^2 + 2i*k0.*(x-d))./( (k0 + kex)^2 + gamma^2 - (x-d).^2 + 2i.*(x-d)*(k0 + kex) )).^2+e*x+f',...
                '$$a\left|\frac{\kappa_0^2-\kappa_{ex}^2+\gamma^2-(x-d)^2+2i\kappa_0(x-d)}{(\kappa_0+\kappa_{ex})^2+\gamma^2-(x-d)^2+2i(x-d)(\kappa_0+\kappa_{ex})}\right|^2+ex+f$$',...
            {'k_0', 'k_ex', 'gamma', 'a','d','e','f'},...
                {'Intrinsic Lw','Extrinsic Lw','Loss','Amplitude','Center','Gradient','Offset'});
