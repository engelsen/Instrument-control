    ...
    methods(Access=private)
        ...
        function createFitStruct(this)
            ...
            addFit(this,'Gorodetsky2000plus',...
                ['(a+(x-b)*c+d*(x-b).^2)*abs( (k0^2/4 - kex^2/4 + gamma^2/4 - (x-b).^2 + i*k0.*(x-b))',...
                './( (k0 + kex)^2/4 + gamma^2/4 - (x-b).^2 + i.*(x-b)*(k0 + kex) )).^2'],...
                ['$$\left[a+c(x-b)+d(x-b)^2\right]\left|\frac{\kappa_0^2/4-\kappa_{ex}^2/4+\gamma^2/4-(x-b)^2+i\kappa_0(x-b)/2}',...
                '{(\kappa_0+\kappa_{ex})^2/4+\gamma^2/4-(x-b)^2+i(x-b)(\kappa_0+\kappa_{ex})}\right|^2$$'],...
                { 'a','b','c','d','gamma','k0', 'kex'},...
                {'Background','Center','Lin. Coeff','Quad. Coeff.',...
                'Mode splitting','Intrinsic','Extrinsic'});
