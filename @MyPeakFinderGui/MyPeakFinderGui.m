classdef MyPeakFinderGui < handle
    ...
    methods
        ...
        function addPeak(this)
            ...
            if checkPeakExist(this.PeakFinder,x_lim(1),x_lim(2))
                opts=struct('WindowStyle','modal','Interpreter','tex');
                errordlg(['A peak already exists in the window. ',...
                    'To add a new peak, zoom such that there is ',...
                    'no peak in the window'],'Error',opts);
                return
            end
            searchPeaks(this.PeakFinder,...
                'ClearPeaks',false,...
