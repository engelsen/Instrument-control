function ProgList = getLocalPrograms()
    ...
    ProgList = struct( ...
        ...
        'run_expr',     {}, ...     % Expression to run 
        'run_bg_expr',  {} ...      % Expression to run without GUI
        );
    ...
    for i = 1:length(instr_names)
        ...
        if isempty(InstrumentList.(nm).control_class)
            continue
        end
        ...
        ProgList(j).info = ['% This entry is automatically generated ' ...
            'from InstrumentList'];
        ...
        if ismethod(InstrumentList.(nm).control_class, 'createLogger')
            
            % Add an entry for starting a logger with this instrument
            ProgList(j) = struct( ...
                'name',         [S.name 'Logger'], ... 
                'title',        [S.name 'Logger'], ...     
                'info',         ['Logger for ' S.name], ...
                'type',         'logger', ...     
                'run_expr',     ['runLogger(' S.name ');'], ...      
                'run_bg_expr',  ['runLogger(' S.name ', ''instr_gui'', false);'] ...      
                );
...
function RunFiles = readRunFiles(dir)
    ...
    for i = 1:length(run_names)
        ...
        if isfield(ParamList, 'comment_header')
            RunFiles(i).info = ParamList.comment_header;
        end
        
        if isfield(ParamList, 'title')
            RunFiles(i).title = ParamList.title;
        end
        
        % Menu title is an old alias for title
        if isfield(ParamList, 'menu_title')
            RunFiles(i).title = ParamList.menu_title;
        end 
        
        if isfield(ParamList, 'run_expr')
            RunFiles(i).run_expr = ParamList.run_expr;
        end
        
        if isfield(ParamList, 'run_bg_expr')
            RunFiles(i).run_expr = ParamList.run_bg_expr;
        end
    end
end
